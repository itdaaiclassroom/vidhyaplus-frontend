...
import { User, Mail, Phone, BookOpen, Layers, Star, Award, Settings, CheckCircle, Shield } from 'lucide-react';
...
export default function PrincipalProfile() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await updatePrincipalProfile(payload);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Left Column: Principal Information */}
            ...
            {editing ? (
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5"><Label>Experience</Label><Input value={form.experience} onChange={e => setForm({...form, experience: e.target.value})} placeholder="e.g. 10 Years" /></div>
                  <div className="space-y-1.5"><Label>Highest Qualification</Label><Input value={form.highest_qualification} onChange={e => setForm({...form, highest_qualification: e.target.value})} placeholder="e.g. M.A. B.Ed." /></div>
                </div>
                <div className="space-y-1.5">
                  <Label>Skills / Expertise (comma separated)</Label>
                  <Input value={form.skills} onChange={e => setForm({...form, skills: e.target.value})} placeholder="e.g. Educational Leadership, School Administration" />
                </div>
                <div className="space-y-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-6">
                ...
                <div>
                  ...
                  <p className="text-base font-medium text-slate-800">{profile?.designation || 'Principal'}</p>
                </div>
                <div>
        ...
        {/* Right Column: School Effectiveness Score */}
        <Card className="border-slate-200 shadow-sm rounded-2xl overflow-hidden flex flex-col">
          <div className="bg-gradient-to-br from-indigo-500 to-purple-600 p-6 text-white text-center flex-1 flex flex-col justify-center items-center">
            ...
            <h3 className="text-xl font-bold mb-1">School Effectiveness Score</h3>
            ...
            <div className="mt-8 bg-white/10 backdrop-blur-md rounded-xl p-4 w-full text-left space-y-2 border border-white/20">
              ...
              <div className="flex justify-between text-sm">
                <span className="text-indigo-50">Quiz Performance</span>
                <span className="font-semibold text-white">30% Weight</span>
              </div>
            </div>
          </div>
        ...
        <CardContent className="p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
            ...
            <div className="bg-white border border-slate-100 rounded-xl p-5 shadow-sm text-center">
              ...
              <p className="text-xs font-semibold text-slate-400 uppercase mt-1">Student Attendance</p>
            </div>
